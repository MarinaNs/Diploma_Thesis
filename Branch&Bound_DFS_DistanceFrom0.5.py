# Code developed by Marina Nousi implementing Branch and Bound with DFS and Distance from 0.5 heuristic.
...
def branch_and_bound(
    ...
    # Iterate through variables z[start_idx:end_idx]
    for idx in range(start_idx, end_idx):

        # Check if the variable is integer-type and its value is not integer
        if integer_var[idx] and not is_nearly_integer(x_candidate[idx]):

            vars_have_integer_vals = False  # Found a fractional variable

            # Compute the distance from 0.5
            distance = abs(x_candidate[idx] - 0.5)

            # Select the variable with the smallest distance to 0.5 (Distance from 0.5 Heuristic)
    ...
    while len(stack) != 0:
        ...
        if model.status != GRB.OPTIMAL:
            ...
            if (
                nodes_per_depth[current_node.depth] == 0
            ):  # έλεγχος αν έχουν εξεταστεί όλοι οι κόμβοι σε ένα επίπεδο
        ...
        if vars_have_integer_vals:
            if isMax:
                ...
                if upper_bound > x_obj:  # Found better solution
                    ...
                    # Remove the children nodes from each next depth -> αποκλείονται κόμβοι απο τα επίπεδα που βρίσκονται πιο κάτω
                    for i in range(current_node.depth + 1, len(nodes_per_depth)):
                        nodes_per_depth[i] -= 2 * (i - current_node.depth)
...
if __name__ == "__main__":
    ...
    results_file = f"results_DFS_DistanceFrom0.5_{os.path.basename(folder_path)}.txt"
